...
def export_glyph(symbol, font_desc, output_dir, icon_name):
    ...
    cr.translate(
        128 - (w * scale / 2) - (ink.x * scale / Pango.SCALE),
    ...
    with open(filename, "w") as f:
        f.write(
            content.replace(
                "rgb(74.509804%, 74.509804%, 74.509804%)", "currentColor"
            ).replace(
                "</svg>", "<style>.symbolic { fill: currentColor; }</style></svg>"
            )
        )
...
def generate_provider_source_file(icon_names):
    src_path = (
        Path(__file__).parent.parent / "src" / "Services" / "SymbIconProvider.vala"
    )
