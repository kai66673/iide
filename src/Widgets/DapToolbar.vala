...
public class Iide.DapToolbar : Gtk.Box {
    ...
    public DapToolbar (WindowSession session) {
        ...
        var symb_provider = SymbIconProvider.get_instance ();

        // 2. Кнопка Start / Continue (Зеленый треугольник)
        this.start_continue_button = new Gtk.Button ();
        this.start_continue_button.icon_name = symb_provider.icon_name (IconID.DAP_START);
        ...
        this.stop_button.icon_name = symb_provider.icon_name (IconID.DAP_STOP);
    ...
    private void on_start_continue_clicked () {
        ...
        if (dap_service.session_state == DapSessionState.EMPTY) {
            ...
            string workspace_root = this.session.current_project_root.get_path ();
