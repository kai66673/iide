...
namespace Iide {
    ...
    public class IdeLspCompletionItem : GLib.Object {
    ...
    public enum IdeLspCompletionKind {
        TEXT = 1,
        METHOD = 2,
        FUNCTION = 3,
        CONSTRUCTOR = 4,
    ...
    public class IdeLspCompletionResult : GLib.Object {
    ...
    public class IdeLspClient : GLib.Object {
        ...
        public async IdeLspCompletionResult ? request_completion (string uri, int line, int character, string? trigger_character = null) {
            ...
            var params = builder.get_root ();

            var response_string = yield send_request ("textDocument/completion", params);

            if (response_string == null) {
                return null;
            }

            return parse_completion_response (uri, response_string);
        }

        private IdeLspCompletionResult ? parse_completion_response (string uri, string response_body) {
            var parser = new Json.Parser ();
            try {
                parser.load_from_data (response_body);
            } catch (Error e) {
                warning ("LSP completion response parse error: %s", e.message);
                return null;
            }
            ...
            if (root.get_node_type () == Json.NodeType.OBJECT) {
                ...
                if (obj.has_member ("items") && obj.get_member ("items").get_node_type () == Json.NodeType.ARRAY) {
                    var items_array = obj.get_member ("items").get_array ();
                    for (int i = 0; i < (int) items_array.get_length (); i++) {
                        var item_node = items_array.get_element (i);
                        var item = parse_completion_item (item_node);
                        if (item != null) {
                ...
                var items_array = root.get_array ();
                for (int i = 0; i < (int) items_array.get_length (); i++) {
                    var item_node = items_array.get_element (i);
                    var item = parse_completion_item (item_node);
                    if (item != null) {
            ...
            debug ("IdeLspClient: Parsed %d completion items for %s", result.items.size, uri);
            completion_received (uri, result);
            return result;
        }

        private IdeLspCompletionItem ? parse_completion_item (Json.Node node) {
            ...
            if (obj.has_member ("detail")) {
                item.detail = obj.get_string_member ("detail");
            }
            if (obj.has_member ("documentation")) {
                item.documentation = obj.get_string_member ("documentation");
            }
            if (obj.has_member ("insertText")) {
                item.insert_text = obj.get_string_member ("insertText");
            } else {
                item.insert_text = item.label;
            }
            if (obj.has_member ("sortText")) {
                var sort_text = obj.get_string_member ("sortText");
                item.sort_text_priority = (int) sort_text.to_int ();
            }
            if (obj.has_member ("insertText")) {
                item.insert_text = obj.get_string_member ("insertText");
            }
            if (obj.has_member ("kind")) {
                item.kind = (IdeLspCompletionKind) (int) obj.get_member ("kind").get_int ();
            }
            if (obj.has_member ("range") && obj.get_member ("range").get_node_type () == Json.NodeType.OBJECT) {
                var range = obj.get_member ("range").get_object ();
                if (range.has_member ("start") && range.get_member ("start").get_node_type () == Json.NodeType.OBJECT) {
                    ...
                    if (start.has_member ("line")) {
                        item.start_line = (int) start.get_member ("line").get_int ();
                    }
                    if (start.has_member ("character")) {
                        item.start_column = (int) start.get_member ("character").get_int ();
                    }
                }
                if (range.has_member ("end") && range.get_member ("end").get_node_type () == Json.NodeType.OBJECT) {
                    ...
                    if (end.has_member ("line")) {
                        item.end_line = (int) end.get_member ("line").get_int ();
                    }
                    if (end.has_member ("character")) {
                        item.end_column = (int) end.get_member ("character").get_int ();
                    }
                }
            }
        ...
        private void handle_message (string body) {
            ...
            if (obj.has_member ("id") && !obj.has_member ("method")) {
                int id = (int) obj.get_member ("id").get_int ();
                if (obj.has_member ("result")) {
                    var result_node = obj.get_member ("result");
                    var result_generator = new Json.Generator ();
                    result_generator.root = result_node;
                    pending_responses.set (id, result_generator.to_data (null));
                } else if (obj.has_member ("error")) {
                    print ("[IdeLspClient] Got error for id=%d\n", id);
                }
                return;
            }
